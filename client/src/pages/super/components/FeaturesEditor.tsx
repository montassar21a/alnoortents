...
export function FeaturesEditor() {
  const { data: items, refetch, isLoading } = trpc.admin.features.list.useQuery();
  const createMutation = trpc.admin.features.create.useMutation();
  const updateMutation = trpc.admin.features.update.useMutation();
  const deleteMutation = trpc.admin.features.delete.useMutation();
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateMutation.mutate({ id: editingId, ...formData }, {
        onSuccess: () => { toast.success("Feature updated!"); resetForm(); refetch(); }
      });
    } else {
      createMutation.mutate(formData, {
        onSuccess: () => { toast.success("Feature created!"); resetForm(); refetch(); }
      });
    }
  };
  ...
  const handleDelete = (id: number) => {
    if (confirm("Are you sure you want to delete this feature?")) {
      deleteMutation.mutate({ id }, {
        onSuccess: () => { toast.success("Feature deleted!"); refetch(); }
      });
    }
  };

  if (isLoading) return <div className="text-slate-400">Loading features...</div>;

  return (
    <div className="space-y-6 mt-8">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-bold text-white">Manage Features</h3>
      ...
      {(isCreating || editingId) && (
        <Card className="bg-slate-900 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.1)]">
          <CardHeader>
            <CardTitle>{editingId ? "Edit Feature" : "Add New Feature"}</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex gap-2 justify-end pt-4">
                ...
                <Button type="submit" className="bg-amber-600 hover:bg-amber-700 text-white">
                  <Save className="w-4 h-4 mr-2" /> Save Feature
                </Button>
              </div>
            </form>
          </CardContent>
